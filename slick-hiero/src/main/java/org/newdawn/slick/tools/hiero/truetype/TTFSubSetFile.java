...
public class TTFSubSetFile extends TTFFile {
    ...
    private void createCvt(FontFileReader in) throws IOException {
        ...
        if (entry != null) {
            pad4();
            seekTab(in, "cvt ", 0);
            System.arraycopy(in.getBytes((int) entry.getOffset(), (int) entry.getLength()), 0, output, currentPos,
                    (int) entry.getLength());
    ...
    private void createGlyf(FontFileReader in, Map glyphs) throws IOException {
        ...
        if (entry != null) {
            ...
            for (int i = 0; i < origIndexes.length; i++) {
                ...
                // Copy glyph
                System.arraycopy(in.getBytes((int) entry.getOffset() + (int) mtxTab[origGlyphIndex].getOffset(),
                        glyphLength), 0, output, currentPos, glyphLength);
    ...
    private void createHmtx(FontFileReader in, Map glyphs) throws IOException {
        ...
        if (entry != null) {
            ...
            while (e.hasNext()) {
                ...
                writeUShort(currentPos + subsetIndex.intValue() * 4, mtxTab[origIndex.intValue()].getWx());
                writeUShort(currentPos + subsetIndex.intValue() * 4 + 2, mtxTab[origIndex.intValue()].getLsb());
            }
                    ...
                    if (in.readTTFShort(entry.getOffset() + mtxTab[origIndex.intValue()].getOffset()) < 0) {
                        ...
                        while (cps.hasNext()) {
                            ...
                            if (glyphs.get(cIdx) == null && newComposites.get(cIdx) == null) {
                                newComposites.put(cIdx, Integer.valueOf(newIndex));
                                newIndex++;
                            }
                        }
                    }
