...
public class TTFFile {
    ...
    private boolean readCMAP(FontFileReader in) throws IOException {
        ...
        /*int cmap_length =*/
        in.readTTFUShort(); //skip cmap length
                ...
                for (int j = cmapStartCounts[i]; j <= cmapEndCounts[i]; j++) {
                    ...
                    if (mtxPtr < mtxTab.length) {
                        ...
                        if (cmapRangeOffsets[i] != 0 && j != 65535) {
                            int glyphOffset = glyphIdArrayOffset + ((cmapRangeOffsets[i] / 2) + (j -
                                    cmapStartCounts[i]) + (i) - cmapSegCountX2 / 2) * 2;
                            ...
                            if (v != null) {
                                ...
                                while (e.hasNext()) {
                                    ...
                                    if (log.isTraceEnabled()) {
                                        log.trace("Added width " + mtxTab[glyphIdx].getWx() + " uni: " + j + " ansi: " +
                                                "" + aIdx.intValue());
                                    }
                                }
                            }
    ...
    public boolean readFont(FontFileReader in, String name) throws IOException {
        ...
        if (!checkTTC(in, name)) {
            if (name == null) {
                throw new IllegalArgumentException("For TrueType collection you must specify which font " + "to " +
                        "select (-ttcname)");
            } else {
    ...
    private void createCMaps() {
        ...
        while (e.hasNext()) {
            ...
            if (((lastMapping.getUnicodeIndex() + 1) != um.getUnicodeIndex()) || ((lastMapping.getGlyphIndex() + 1)
                    != um.getGlyphIndex())) {
    ...
    private final void readPostScript(FontFileReader in) throws IOException {
        ...
        switch (postFormat) {
            ...
            case 0x00020000:
                ...
                if (log.isDebugEnabled()) {
                    log.debug("Reading " + numGlyphStrings + " glyphnames, that are not in the standard Macintosh" +
    ...
    protected final void readIndexToLocation(FontFileReader in) throws IOException {
        if (!seekTab(in, "loca", 0)) {
            throw new IOException("'loca' table not found, happens when the font file doesn't" + " contain TrueType " +
                    "outlines (trying to read an OpenType CFF font maybe?)");
        }
        for (int i = 0; i < numberOfGlyphs; i++) {
            mtxTab[i].setOffset(locaFormat == 1 ? in.readTTFULong() : (in.readTTFUShort() << 1));
        }
        lastLoca = (locaFormat == 1 ? in.readTTFULong() : (in.readTTFUShort() << 1));
    }
    ...
    private final void readGlyf(FontFileReader in) throws IOException {
        ...
        for (int i = 0; i < (numberOfGlyphs - 1); i++) {
            if (mtxTab[i].getOffset() != mtxTab[i + 1].getOffset()) {
                ...
                in.skip(2);
                final int[] bbox = {in.readTTFShort(), in.readTTFShort(), in.readTTFShort(), in.readTTFShort()};
    ...
    private final void readName(FontFileReader in) throws IOException {
        ...
        while (n-- > 0) {
            ...
            if (((platformID == 1 || platformID == 3) && (encodingID == 0 || encodingID == 1)) && (k == 1 || k == 2
                    || k == 0 || k == 4 || k == 6)) {
                ...
                if (!notice.equals("") && !fullName.equals("") && !fontName.equals("") && !familyName.equals("") &&
    ...
    private void determineAscDesc() {
        ...
        if (ascender - descender > upem) {
            log.warn("Ascender and descender together are larger than the em box." + " This could lead to a wrong " +
                    "baseline placement in Apache FOP.");
        }
    }
    ...
    private void guessVerticalMetricsFromGlyphBBox() {
        ...
        log.debug("Ascender from glyph 'd': " + localAscender + " " + (localAscender));
        log.debug("Descender from glyph 'p': " + localDescender + " " + (localDescender));
        if (ascender - descender > upem) {
            log.debug("Replacing specified ascender/descender with derived values to get values" + " which fit in the" +
                    " em box.");
            ascender = localAscender;
            descender = localDescender;
        }

        log.debug("xHeight from glyph 'x': " + localXHeight + " " + (localXHeight));
        log.debug("CapHeight from glyph 'H': " + localCapHeight + " " + (localCapHeight));
        if (capHeight == 0) {
    ...
    public void printStuff() {
        ...
        System.out.println("FontBBox:    [" + (fontBBox1) + " " + (fontBBox2) + " " + (fontBBox3) + " " + (fontBBox4)
                + "]");
    }
    ...
    private Integer unicodeToGlyph(int unicodeIndex) throws IOException {
        final Integer result = (Integer) unicodeToGlyphMap.get(Integer.valueOf(unicodeIndex));
        if (result == null) {
