...
public class BMFontUtil {
    ...
    public void save(File outputBMFontFile, boolean flip, boolean antiAliasing) throws IOException, SlickException {
        ...
        out.println("info face=\"" + font.getFontName() + "\" size=" + font.getSize() + " bold=" + (font.isBold() ? 1
                : 0) + " italic=" + (font.isItalic() ? 1 : 0) + " charset=\"\" unicode=0 stretchH=100 smooth=1 aa=" +
                (antiAliasing ? 1 : 0) + " padding=0,0,0,0 spacing=0,0");
        out.println("common lineHeight=" + lineHeight + " base=" + unicodeFont.getAscent() + " scaleW=" + pageWidth +
                " scaleH=" + pageHeight + " pages=" + unicodeFont.getGlyphPages().size() + " packed=0 ascent=" +
                unicodeFont.getAscent() + " descent=" + unicodeFont.getDescent() + " leading=" + unicodeFont
                .getLeading());
        ...
        out.println("char id=32   x=0     y=0     width=0     height=0     xoffset=0     yoffset=" + unicodeFont
                .getAscent() + "    xadvance=" + xAdvance + "     page=0  chnl=0 ");
        ...
        for (Iterator pageIter = unicodeFont.getGlyphPages().iterator(); pageIter.hasNext(); ) {
            ...
            for (Iterator glyphIter = page.getGlyphs().iterator(); glyphIter.hasNext(); ) {
                ...
                out.println("char id=" + glyph.getCodePoint() + "   " + "x=" + (int) (glyph.getImage()
                        .getTextureOffsetX() * pageWidth) + "     y=" + (int) (glyph.getImage().getTextureOffsetY() *
                        pageHeight) + "     width=" + glyph.getWidth() + "     height=" + glyph.getHeight() + "     " +
                        "xoffset=" + xOffset + "     yoffset=" + glyph.getYOffset() + "    xadvance=" + xAdvance + " " +
        ...
        if (ttfFileRef == null) {
            ...
            for (Iterator iter = allGlyphs.iterator(); iter.hasNext(); ) {
                ...
                glyphCodeToCodePoint.put(Integer.valueOf(getGlyphCode(font, glyph.getCodePoint())),
                        Integer.valueOf(glyph.getCodePoint()));
            }
            ...
            for (Iterator iter = kernings.iterator(); iter.hasNext(); ) {
                ...
                out.println("kerning first=" + pair.firstCodePoint + "  second=" + pair.secondCodePoint + "  amount="
                        + pair.offset);
            }
        }
        out.close();
