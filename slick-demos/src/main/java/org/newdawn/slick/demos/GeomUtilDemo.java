...
/**
 * A Demo to try shape cutting
 * <p/>
 *
 * @author Kevin Glass
 */
public class GeomUtilDemo extends BasicGame implements GeomUtilListener {
    ...
    public static void main(String[] argv) {
        try {
            AppGameContainer container = new AppGameContainer(new GeomUtilDemo());
    ...
    public void mouseWheelMoved(int change) {
        if (dynamic) {
            if (change < 0) {
                cut = cut.transform(Transform.createRotateTransform((float) Math.toRadians(10), cut.getCenterX(),
                        cut.getCenterY()));
            } else {
                cut = cut.transform(Transform.createRotateTransform((float) Math.toRadians(-10), cut.getCenterX(),
                        cut.getCenterY()));
