 ...
 * @author kevin
 */
public class ImageCornerDemo extends BasicGame {
     ...
     */
    public ImageCornerDemo() {
        super("Image Corner Demo");
    }
    ...
    public void init(GameContainer container) throws SlickException {
        image = new Image("org/newdawn/slick/testdata/logo.png");

        width = image.getWidth() / 3;
        height = image.getHeight() / 3;

        images = new Image[]{image.getSubImage(0, 0, width, height), image.getSubImage(width, 0, width, height),
                image.getSubImage(width * 2, 0, width, height), image.getSubImage(0, height, width, height),
                image.getSubImage(width, height, width, height), image.getSubImage(width * 2, height, width, height),
    ...
    public static void main(String[] argv) {
        boolean sharedContextDemo = false;

        try {
            AppGameContainer container = new AppGameContainer(new ImageCornerDemo());
