...
public class Input {
    ...
    public boolean isControllerLeft(int controller) {
        ...
        return ((Controller) controllers.get(controller)).getXAxisValue() < -0.5f || ((Controller) controllers.get
                (controller)).getPovX() < -0.5f;
    }
    ...
    public boolean isControllerUp(int controller) {
        ...
        return ((Controller) controllers.get(controller)).getYAxisValue() < -0.5f || ((Controller) controllers.get
                (controller)).getPovY() < -0.5f;
    }
    ...
    public void initControllers() throws SlickException {
        ...
        try {
            ...
            if (e.getCause() instanceof ClassNotFoundException) {
                throw new SlickException("Unable to create controller - no jinput found - add jinput.jar to your " +
                        "classpath");
            }
        ...
        while (Mouse.next()) {
            if (Mouse.getEventButton() >= 0) {
                if (Mouse.getEventButtonState()) {
                    ...
                    if ((pressedX != -1) && (pressedY != -1) && (Math.abs(pressedX - releasedX) <
                            mouseClickTolerance) && (Math.abs(pressedY - releasedY) < mouseClickTolerance)) {
