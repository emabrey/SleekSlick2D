...
public class TiledMap {
     ...
     */
    protected void renderIsometricMap(int x, int y, int sx, int sy, int width, int height, Layer layer,
                                      boolean lineByLine) {
        ...
        while (!allProcessed) {
            ...
            int min = 0;
            if (height > width) {
                min = (startLineTileY < width - 1) ? startLineTileY : (width - currentTileX < height) ? width -
                        currentTileX - 1 : width - 1;
            } else {
                min = (startLineTileY < height - 1) ? startLineTileY : (width - currentTileX < height) ? width -
                        currentTileX - 1 : height - 1;
            }
            ...
            for (int burner = 0; burner <= min; currentTileX++, currentTileY--, burner++) {
                for (int layerIdx = 0; layerIdx < drawLayers.size(); layerIdx++) {
                    ...
                    currentLayer.render(currentLineX, initialLineY, currentTileX, currentTileY, 1, 0, lineByLine,
