    ...
    public TileSet(TiledMap map, Element element, boolean loadImage) throws SlickException {
        ...
        if ((source != null) && (!source.equals(""))) {
            try {
                ...
                Log.error(e);
                throw new SlickException("Unable to load or parse sourced tileset: " + this.map.tilesLocation + "/" +
                        source);
            }
        }
        ...
        if (tileWidthString.length() == 0 || tileHeightString.length() == 0) {
            throw new SlickException("TiledMap requires that the map be created with tilesets that use a " + "single " +
