...
public class Graphics {
    ...
    public Graphics(int width, int height) {
        if (DEFAULT_FONT == null) {
            AccessController.doPrivileged(new PrivilegedAction() {
                public Object run() {
                    try {
                        DEFAULT_FONT = new AngelCodeFont("org/newdawn/slick/data/defaultfont.fnt",
    ...
    /**
     * Clear the state of the alpha map across the entire screen. This sets alpha to 0 everywhere, meaning in {@link
     * Graphics#MODE_ALPHA_BLEND} nothing will be drawn.
     */
    ...
    public void drawLine(float x1, float y1, float x2, float y2) {
        ...
        if (LSR.applyGLLineFixes()) {
            if (x1 == x2) {
                ...
                fillRect(x1 - (lineWidth / 2.0f), y1 - (lineWidth / 2.0f), lineWidth + step,
                ...
                fillRect(x1 - (lineWidth / 2.0f), y1 - (lineWidth / 2.0f), (x2 - x1) + lineWidth + step,
                        lineWidth + step);
                return;
            }
        }
    ...
    public Color getPixel(int x, int y) {
        ...
        return new Color(translate(readBuffer.get(0)), translate(readBuffer.get(1)), translate(readBuffer.get(2)),
