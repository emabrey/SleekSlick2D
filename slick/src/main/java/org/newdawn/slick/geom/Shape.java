...
public abstract class Shape implements Serializable {
    ...
    public boolean contains(float x, float y) {
        ...
        for (int i = 0; i < npoints; i += 2) {
            ...
            if ((xnew < x) == (x <= xold) /* edge "open" at one end */ && ((double) y - (double) y1) * (x2 - x1) < (
        ...
        for (int i = 0; i < length; i += 2) {
            ...
            for (int j = 0; j < thatLength; j += 2) {
                ...
                unknownA = (((points[iNext] - points[i]) * (double) (thatPoints[j + 1] - points[i + 1])) - (
                        (points[iNext + 1] - points[i + 1]) * (thatPoints[j] - points[i]))) / (((points[iNext + 1] -
                        points[i + 1]) * (thatPoints[jNext] - thatPoints[j])) - ((points[iNext] - points[i]) *
                        (thatPoints[jNext + 1] - thatPoints[j + 1])));
                unknownB = (((thatPoints[jNext] - thatPoints[j]) * (double) (thatPoints[j + 1] - points[i + 1])) - (
                        (thatPoints[jNext + 1] - thatPoints[j + 1]) * (thatPoints[j] - points[i]))) / (((points[iNext
                        + 1] - points[i + 1]) * (thatPoints[jNext] - thatPoints[j])) - ((points[iNext] - points[i]) *
                        (thatPoints[jNext + 1] - thatPoints[j + 1])));
    ...
    protected void calculateRadius() {
        ...
        for (int i = 0; i < points.length; i += 2) {
            float temp = ((points[i] - center[0]) * (points[i] - center[0])) + ((points[i + 1] - center[1]) *
                    (points[i + 1] - center[1]));
