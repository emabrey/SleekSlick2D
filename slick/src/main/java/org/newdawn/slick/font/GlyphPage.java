...
public class GlyphPage {
     ...
     */
    private static BufferedImage scratchImage = new BufferedImage(MAX_GLYPH_SIZE, MAX_GLYPH_SIZE,
    ...
    public int loadGlyphs(List glyphs, int maxGlyphsToLoad) throws SlickException {
        ...
        for (Iterator iter = getIterator(glyphs); iter.hasNext(); ) {
            ...
            if (i == maxGlyphsToLoad) {
                // If loading incrementally, flip orderAscending so it won't change,
                // since we'll probably load the rest next time.
        ...
        // Every other batch of glyphs added to a page are sorted the opposite way to attempt to keep same size glyps
        // together.
        orderAscending = !orderAscending;
