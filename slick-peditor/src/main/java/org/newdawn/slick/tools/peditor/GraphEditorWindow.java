...
public class GraphEditorWindow extends JPanel {
    ...
    private DefaultPanel createTopPanel() {
        ...
        properties.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent event) {
                ...
                LinearInterpolator currentValue = (LinearInterpolator) values.get(item);
    ...
    public void setFirstProperty() {
        if (properties.getItemCount() > 0) {
            ...
            LinearInterpolator currentValue = (LinearInterpolator) values.get(properties.getSelectedItem());
            panel.setInterpolator(currentValue);
        }
    }
    ...
    public class GraphPanel extends JPanel {
        ...
        public GraphPanel() {
            ...
            this.addMouseListener(new MouseListener() {
                ...
                public void mouseClicked(MouseEvent e) {
                    mousePressedEvent(e.getX(), e.getY(), e.getButton() == MouseEvent.BUTTON1,
        ...
        private BufferedImage loadBackgroundImage() {
            InputStream in = ParticleEditor.class.getClassLoader().getResourceAsStream("charlie.png");
        ...
        public void mouseDraggedEvent(int x, int y, boolean shiftDown) {
            ...
            if (idx != -1) {
                ...
                if (selected == curve.get(0) || selected == curve.get(curve.size() - 1)) {
                    selected.y = world.y;
                } else {
                    selected.x = world.x;
                    selected.y = world.y;
        ...
        public void mouseMovedEvent(int x, int y) {
            ...
            for (int i = 0; i < curve.size(); i++) {
                ...
                if ((posView.x > (pView.x - 12 / 2)) && (posView.x < (pView.x + 12 / 2)) && (posView.y > (pView.y -
                        12 / 2)) && (posView.y < (pView.y + 12 / 2))) {
        ...
        public void mousePressedEvent(int x, int y, boolean leftButton, boolean rightButton, boolean middleButton) {
            ...
            if (rightButton) {
                ...
                if (idx == -1) {
                    ...
                    if (x >= viewportMinX && x <= viewportMaxX && y >= viewportMaxY && y <= viewportMinY) {
         ...
         */
        private void drawLegend(Graphics2D g, float vx0, float vy0, float vx1, float vy1) {
            g.setColor(COLOR_BACKGROUND);
            g.fillRect((int) vx0, (int) (vy1 - viewBorderY), (int) (vx1 - vx0 + viewBorderX),
            ...
            g.fillRect((int) vx0, (int) vy0, (int) (vx1 - vx0 + viewBorderX), (int) (vy0));

            // x line bottom (min y)
            g.setColor(COLOR_LEGEND);
            g.drawLine((int) vx0, (int) vy0, (int) (vx1 + viewBorderX), (int) vy0);
            g.drawLine((int) vx0, (int) vy0 + 1, (int) (vx1 + viewBorderX), (int) vy0 + 1);
            ...
            for (int x = 0; x < intervalCountX + 1; x++) {
                ...
                g.setColor(COLOR_LEGEND);
                g.drawLine((int) c.x, (int) c.y, (int) c.x, (int) c.y + legendLineMain);

                g.setColor(COLOR_LEGEND_GRID);
                g.drawLine((int) c.x, (int) (viewportMaxY - viewBorderY), (int) (c.x), (int) (viewportMinY));
                ...
                if (x <= (intervalCountX - 1)) {
                    for (int xsub = 0; xsub < 2; xsub++) {
                        float xi1 = (worldMaxX - worldMinX) / intervalCountX * (x + 1);
                        CurvePoint ci = worldToView(new CurvePoint((xi0 + xi1) / 2.0f * xsub, 0));
                        ci.y = vy0;

                        g.setColor(COLOR_LEGEND);
                        g.drawLine((int) ci.x, (int) ci.y, (int) ci.x, (int) ci.y + legendLineMin);
                    }
                }
            }
            ...
            // y
            g.setColor(COLOR_LEGEND_BACKGROUND);
            g.fillRect(0, (int) (vy1 - viewBorderY), (int) vx0, (int) (vy0 + viewBorderY));
            ...
            for (int y = 0; y < intervalCountY + 1; y++) {
                float yi0 = (worldMaxY - worldMinY) / intervalCountY * y + worldMinY;
                CurvePoint c = worldToView(new CurvePoint(0, yi0));

                // legend line
                g.setColor(COLOR_LEGEND);
                g.drawLine((int) c.x - legendLineMainY, (int) c.y, (int) c.x, (int) c.y);

                // grid line
                g.setColor(COLOR_LEGEND_GRID);
                g.drawLine((int) c.x, (int) (c.y), (int) (c.x + viewportMaxX), (int) (c.y));

                // legend text
                String text = convertFloat(yi0);

                g.setColor(COLOR_LEGEND);
                g.drawString(text, (int) vx0 - g.getFontMetrics().stringWidth(text) - 2, (int) (c.y - 2));
            }

            // x line top (max y)
            g.setColor(COLOR_LEGEND);
            g.drawLine((int) vx0, (int) (vy1), (int) (vx1 + viewBorderX), (int) (vy1));

            // left
            g.drawLine((int) vx0, (int) vy0, (int) vx0, (int) (vy1 - viewBorderY));

            // right
            g.drawLine((int) vx1, (int) vy0, (int) vx1, (int) (vy1 - viewBorderY));
        }
        ...
        public void paint(Graphics2D g, float vx0, float vy0, float vx1, float vy1) {
            if (!isActive()) {
                g.drawImage(backgroundImage, 0, (int) vy1 - backgroundImage.getHeight(), null);
                return;
            }
            ...
            drawLegend(g, viewportMinX, viewportMinY, viewportMaxX, viewportMaxY);
            ...
            for (int i = 0; i < curve.size(); i++) {
                ...
                g.fillRect(xPoints[i] - width / 2 - off, yPoints[i] - height / 2 - off, width + off * 2,
                        height + off * 2);

                g.setColor(color);
                g.fillRect(xPoints[i] - width / 2 - off + 1, yPoints[i] - height / 2 - off + 1, width + off * 2 - 2,
                        height + off * 2 - 2);

                if (value != null && mouseX >= viewportMinX && mouseX <= viewportMaxX && mouseY >= vy0 && mouseY <=
                        vy1) {
                    ...
                    g.drawLine((int) p.getX(), (int) vy0, (int) p.getX(), (int) vy1);

                    // update mouse position labels
                    valueXLabel.setText(TEXT_CURRENT_X + convertFloat(crossPoint.getX()));
                    valueYLabel.setText(TEXT_CURRENT_Y + convertFloat(crossPoint.getY()));
    ...
    public static void main(String[] argv) {
        ...
        LinearInterpolator test = new ConfigurableEmitter("bla").new LinearInterpolator(curve, 0, 255);
        bottom.registerValue(test, "Test");

        curve = new ArrayList();
        curve.add(new Vector2f(0.0f, 255.0f));
        curve.add(new Vector2f(1.0f, 0.0f));
        test = new ConfigurableEmitter("bla").new LinearInterpolator(curve, 0, 255);
